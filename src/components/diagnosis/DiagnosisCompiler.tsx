...
type DiagnosisCompilerProps =
  ...
  | {
      ...
      builderDefinition?: BuilderDiagnosisDefinition;
    };
...
const ResultLayoutRenderer = ({
  ...
  if (layout === "compatibility_radar") {
    const entries = LEGAL_TRAIT_KEYS.slice(0, 4).map(
      (traitKey) => [traitKey, vector[traitKey]] as const,
    );
...
export const DiagnosisCompiler = (props: DiagnosisCompilerProps) => {
  ...
  const plugDefinition = isBuilderMode
    ? null
    : (programDefinition as PlugDiagnosisDefinition);
  ...
  const handleStart = useCallback(() => {
    ...
    if (shouldTrackSpecialtyBridgeDeepPlay(ref, programDefinition.slug)) {
      trackDiagnosisEvent(
        "specialty_deep_play_start",
        buildSpecialtyDeepPlayStartPayload(programDefinition.slug),
      );
    }
