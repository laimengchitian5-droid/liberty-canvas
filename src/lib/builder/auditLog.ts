...
export type BuilderAuditAction =
  | "save_draft"
  | "publish"
  | "unpublish"
  | "delete";
...
export async function appendBuilderAuditEntry(
...
export async function listBuilderAuditEntries(
  limit = 50,
...
export async function listBuilderAuditBySlug(
  ...
  const log = await readAuditLog();
  return log.filter((entry) => entry.slug === slug).slice(-limit).reverse();
}
